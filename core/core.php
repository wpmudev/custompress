...
class CustomPress_Core {
	...
	function CustomPress_Core() {
		...
		add_action('admin_head', array($this, 'on_print_scripts'));
	...
	*/
	...
	function on_print_scripts(){
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){jQuery('.pickdate').datepicker()});
			function update_stylesheet(url){new_stylesheet=url;if(jQuery("#cp_dynamic_css").length==0){jQuery("head").append("<link>");css=jQuery("head").children(":last");css.attr({id:"cp_dynamic_css",rel:"stylesheet",type:"text/css",href:new_stylesheet})}else{jQuery("#cp_dynamic_css").attr("href",new_stylesheet)}}
			</script>
		...
		function ajax_actions() { ?>
			...
			*/
			function jquery_ui_css(){
				echo '<script type="text/javascript">update_stylesheet( "' . $this->plugin_url . 'css/' . $this->get_options('datepicker_theme') .'/datepicker.css"); </script>' . PHP_EOL;
			}
