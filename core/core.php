...
class CustomPress_Core {
	...
	*/
	...
	function on_enqueue_scripts(){
		...
		//longer exceptions
		$lang = (in_array(WPLANG, array('ar_DZ', 'cy_GB', 'en_AU', 'en_GB', 'en_NZ', 'fr_CH', 'nl_BE', 'pt_BR', 'sr_SR', 'zh_CN', 'zh_HK', 'zh_TW') ) )  ?  WPLANG : $lang;
	...
	function on_admin_head(){
		...
		<script type="text/javascript">function update_stylesheet(url){new_stylesheet=url;if(jQuery("#cp_dynamic_css").length==0){jQuery("head").append("<link>");css=jQuery("head").children(":last");css.attr({id:"cp_dynamic_css",rel:"stylesheet",type:"text/css",href:new_stylesheet})}else{jQuery("#cp_dynamic_css").attr("href",new_stylesheet)}}</script>
